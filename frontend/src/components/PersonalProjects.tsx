...
const PersonalProjects: React.FC = () => {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % personalProjects.length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + personalProjects.length) % personalProjects.length);
  };
  ...
  return (
    <Box 
      ref={ref}
      sx={{ 
        background: "linear-gradient(135deg, #33CFCB 0%, #2bb8b3 100%)", 
        py: 8,
        position: "relative",
        overflow: "hidden",
        "&::before": {
